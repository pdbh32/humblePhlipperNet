...
def _ingest_wiki_data(endpoint: str, timestamp: int) -> None:
    if market_data_storage.get_path(endpoint, timestamp).exists(): return market_data_storage.load(endpoint, timestamp)
    return wiki.fetch(endpoint, timestamp)

def _update_wiki_cache(endpoint: str, int_secs: int, T: int) -> None:
    existing_ts = market_data_cache.get(endpoint).keys()
...
def _init_quotes_cache() -> None:
    ...
    one_h = market_data_cache.get_df("1h")
    latest = market_data_cache.get_df("latest")
